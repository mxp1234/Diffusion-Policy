...
def convert_custom_to_zarr(input_path, output_zarr_path, shape_meta, env_meta):
    ...
    for demo_dir in tqdm(demo_dirs, desc="Processing demo folders"):
        ...
        for pkl_idx, pkl_file in enumerate(pkl_files):
            ...
            # Collect lowdim data including plastic apple info
            ...
            # Merge joint_positions_dummy with plastic_apple_position for action
            if 'joint_positions_dummy' in data and 'plastic_apple_position' in data:
                joint_pos = np.array(data['joint_positions_dummy'], dtype=np.float32)
                apple_pos = np.array(data['plastic_apple_position'], dtype=np.float32)
                action_value = np.concatenate([joint_pos, apple_pos])
                if action_value.shape != shape_meta['action']['joint_positions_dummy']['shape']:
                    raise ValueError(f"Shape mismatch for action in {pkl_file}: "
                                   f"got {action_value.shape}, expected {shape_meta['action']['joint_positions_dummy']['shape']}")
                all_data['action'].append(action_value)
            else:
                missing_key = 'joint_positions_dummy' if 'joint_positions_dummy' not in data else 'plastic_apple_position'
                raise KeyError(f"Required key {missing_key} not found in {pkl_file}")

            # Collect and resize images
            for cam_idx, rgb_key in enumerate(rgb_keys, 1):
                cam_dir = 'view_5' if cam_idx == 1 else 'view_254'
                ...
                if os.path.exists(img_path):
                    ...
                    expected_shape = shape_meta['obs'][rgb_key]['shape']
    ...
    for key in ['joint_positions', 'joint_positions_dummy', 'state', 'plastic_apple_position', 'plastic_apple_orientation', 'image_1', 'image_2', 'action']:
        arr = root['data'][key]
        print(f"Loaded {key} shape: {arr.shape}")
        if key in ['joint_positions', 'joint_positions_dummy', 'state', 'plastic_apple_position', 'plastic_apple_orientation', 'action']:
...
if __name__ == "__main__":
    shape_meta = {
        'obs': {
            ...
            'plastic_apple_position': {'shape': (3,), 'type': 'low_dim'},
            'plastic_apple_orientation': {'shape': (4,), 'type': 'low_dim'},
            'image_1': {'shape': (3, 240, 320), 'type': 'rgb'},
            'image_2': {'shape': (3, 240, 320), 'type': 'rgb'},
        },
        'action': {
            'joint_positions_dummy': {'shape': (22,)}  # 19 + 3 from plastic_apple_position
        }
    }
    ...
    output_zarr_path = "/root/code/diffusion_policy/data/real/bc_data_il/gello_all_apple.zarr"
